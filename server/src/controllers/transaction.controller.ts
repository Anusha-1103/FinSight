...
export class TransactionController {
  ...
  static async createTransaction(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      await AuditService.log(userId, 'CREATE_TRANSACTION', 'Transaction', transaction.id, { amount: numericAmount, type, description }, req.ip, req.headers['user-agent']);
  ...
  static async updateTransaction(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      await AuditService.log(userId, 'UPDATE_TRANSACTION', 'Transaction', id, { amount: updated.amount, type: updated.type }, req.ip, req.headers['user-agent']);
  ...
  static async deleteTransaction(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      await AuditService.log(userId, 'DELETE_TRANSACTION', 'Transaction', id, { amount: existing.amount }, req.ip, req.headers['user-agent']);
  ...
  static async scanReceipt(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (file) {
        imageResult = await CloudinaryService.uploadImageBuffer(file.buffer, 'receipts');
        parsedData = await AIService.parseReceiptContent(file.buffer);
      } else {
        parsedData = await AIService.parseReceiptContent(rawText || 'Supermarket Receipt $42.50');
      }
