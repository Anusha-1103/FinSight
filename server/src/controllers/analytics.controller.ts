...
export class AnalyticsController {
  static async getAnalytics(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const months: Array<{ label: string; key: string; start: Date; end: Date }> = [];
      ...
      const categorySpentMap = new Map<string, { name: string; color: string; amount: number }>();
      ...
      const spendingByCategoryRaw = Array.from(categorySpentMap.values()).sort(
      ...
      // Income Sources
      const incomeCategoryMap = new Map<string, { name: string; color: string; amount: number }>();
      for (const t of periodTransactions) {
        if (t.type === 'INCOME') {
          const catName = t.category?.name || 'Other';
      ...
      const incomeSourcesRaw = Array.from(incomeCategoryMap.values()).sort(
        (a, b) => b.amount - a.amount
      );
