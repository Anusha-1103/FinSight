...
async function resolveSubscriptionsCategory(userId: string) {
...
export class SubscriptionController {
  static async getSubscriptions(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
  ...
  static async createSubscription(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const userId = req.user!.userId;
      ...
      await AuditService.log(userId, 'CREATE_SUBSCRIPTION', 'Subscription', sub.id, { name: sub.name, amount: sub.amount }, req.ip, req.headers['user-agent']);
  ...
  static async updateSubscription(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const userId = req.user!.userId;
      const { id } = req.params;
  ...
  static async recordPayment(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      await AuditService.log(
        ...
        'Subscription',
        id,
        { amount: sub.amount, accountId, categoryId: subCategory.id, transactionId: result.transaction.id },
        req.ip,
  ...
  static async deleteSubscription(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const userId = req.user!.userId;
      const { id } = req.params;
